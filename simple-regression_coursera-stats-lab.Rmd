...
My smallest Sum of Squares:  124165.3
Actual least Sum of Squares (adding in leastSquares = TRUE value): Sum of Squares: 123721.9
...
\[
  \widehat{runs} = -2789.2429 + 0.6305 \times at\_bats
\]
...
The scatterplot above and the regression output below both suggest that strikeouts -- while evincing a moderate linear negative correlation with runs -- are not as strong a predictive analytic variable as at_bats. The $R^2$ value for runs as a function of strikeouts suggests that only 17% of the variability in runs can be explained by the number of strikeouts, while the $R^2$ for runs ~ at_bats is 37%.
...
Batting average ($R^2 = 0.6561$) just edges out hits ($R^2 = 0.6419$) as the best predictor of runs, according to the linear regression models built to compare both sets of correlations.

```{r runs-vs-others}
# type your code for Question 9 here, and Knit

summary(lm(data = mlb11, runs ~ bat_avg))

ggplot(data = mlb11, aes(x = strikeouts, y = bat_avg)) +
  geom_point() +
  stat_smooth(method = "lm")
...
<ol>
<li> on-base plus slugging (`new_obs`) -- $R^ = 0.9349&</li>
<li> slugging percentage (`new_slug`) -- $R^ = 0.8969&</li> 
<li> on-base percentage (`new_onbase`) -- $R^ = 0.8491&</li>
</ol>
```{r runs-vs-new-vars}
# type your code for Question 10 here, and Knit

summary(lm(data = mlb11, runs ~ new_obs))

ggplot(data = mlb11, aes(x = strikeouts, y = new_obs)) +
  geom_point() +
  stat_smooth(method = "lm")
...
# Checking Normality with a Normal Probability Chart
